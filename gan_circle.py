...
class MainThread():
    def __init__(self):
        ...
        self.parser.add_argument("--warmup_batches", type=int, default=500,
        ...
        self.parser.add_argument("--validation_dataset_name", type=str, default="validation",
    ...
    def analyze(self, gen_h, imgs_hr, opt, hr_shape, loss_pixel_h, loss_content_h, loss_G_h, loss_G, loss_D, dir):
        ...
        ux_mssim = F.conv2d(gen_h, weight=gw, stride=1, padding=opt.window_size // 2, groups=1)
        uy_mssim = F.conv2d(imgs_hr, weight=gw, stride=1, padding=opt.window_size // 2, groups=1)

        ux_mssim2 = ux_mssim.pow(2)
        uy_mssim2 = uy_mssim.pow(2)

        ux2_mssim = F.conv2d(gen_h * gen_h, weight=gw, stride=1, padding=opt.window_size // 2, groups=1)
        uy2_mssim = F.conv2d(imgs_hr * imgs_hr, weight=gw, stride=1, padding=opt.window_size // 2, groups=1)
        uxy_mssim = F.conv2d(gen_h * imgs_hr, weight=gw, stride=1, padding=opt.window_size // 2, groups=1)
        ...
        gen_h_ex = F.conv2d(gen_h, bias=torch.randn(1).to(device), weight=mat, stride=1, padding=opt.window_size // 2,
                            groups=1)
        imgs_hr_ex = F.conv2d(imgs_hr, bias=torch.randn(1).to(device), weight=mat, stride=1,
                              padding=opt.window_size // 2, groups=1)
        ...
        mat_mssim = ((2 * ux_mssim * uy_mssim + c1) * (2 * sigmaxy_mssim + c2)) / (
    ...
    def srgan(self):
        ...
        for epoch in range(self.opt.epoch, self.opt.n_epochs):
            for i, imgs in enumerate(dataloader):
                ...
                save_image(imgs_lr, "./lr_groundtruth/%d.png" % batches_done, normalize=True)
                save_image(imgs_hr, "./hr_groundtruth/%d.png" % batches_done, normalize=True)
                save_image(gen_hr, "./srgan_hr_generated/%d.png" % batches_done, normalize=True)
            ...
            for a, imgsv in enumerate(dataloaderv):
                ...
                with open('./validation/srgan/text/%s' % str(epoch)) as file:
                    file.write(ssim + '\n')
    ...
    def esrgan(self):
        ...
        for epoch in range(opt.epoch, opt.n_epochs):
            for i, imgs in enumerate(dataloader):
                ...
                save_image(gen_h, "./esrgan_hr_generated/%d" % batches_done, normalize=False)
            ...
            for a, imgsv in enumerate(dataloaderv):
                ...
                genv_h = g(imgsv_l)
                genv_h = make_grid(genv_h, nrow=1, noemalize=True)
                save_image(genv_h, "./validation/esrgan/pic/%s" % str(epoch))
